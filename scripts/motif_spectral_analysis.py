...
class UndirectedMotifs:
    ...
    def __init__(self):
        ...
        # Create basic motifs
        # Motif 1: Three edges (triangle/complete graph)
        triangle = nx.Graph()
        ...
        # Motif 2: Four edges (cycle/square)
        square = nx.Graph()
        ...
        # Motif 4: Five edges (cycle with chord)
        ...
        # Motif 5: Six edges (complete graph)
        complete4 = nx.Graph()
    ...
    def compute_optimal_k_eigengap(self, adj_matrix, max_k=20):
        ...
        # Find the index of the largest gap, starting from the second gap
        # to ensure we never find just one cluster
        # Add 2 because we're looking at gaps starting from the second gap
        # and we skipped the first eigenvalue
        if len(gaps) > 1:
            optimal_k = np.argmax(gaps[1:]) + 2
        else:
            # If there's only one gap, use 2 clusters as a fallback
            optimal_k = 2
    ...
    def compute_optimal_k_elbow(self, adj_matrix, max_k=20):
        """
        Compute the optimal number of clusters using the Elbow (Inertia) Method.

        The Elbow Method runs k-means clustering for a range of k values and
        plots the sum of squared distances (inertia) for each k. The optimal k
        is found at the "elbow point" where adding more clusters doesn't 
        ...
        Note:
            This method requires the 'kneed' package. Install it with:
            pip install kneed
        """
        ...
        try:
            ...
            print("Error: The 'kneed' package is required for the Elbow Method.")
            print("Please install it with: pip install kneed")
            print("Falling back to a simple heuristic for finding the elbow point.")
            # Set kneed to None to indicate it's not available
            KneeLocator = None
        ...
        inertias = []
        k_values = range(2, max_k + 1)  # Start from 2 to avoid single-cluster solutions
        ...
        for k in k_values:
            ...
            kmeans.fit(X[:, :k-1])  # Use appropriate number of eigenvectors
            inertias.append(kmeans.inertia_)

        # If we have too few points, return a default value
        if len(inertias) < 3:
            print("Not enough data points to determine optimal k. Using default k=2.")
            return 2
        ...
        if KneeLocator is not None:
            try:
                ...
                print(f"KneeLocator failed with error: {e}")
                print("Falling back to a simple heuristic for finding the elbow point.")
                optimal_k = None
        else:
            optimal_k = None

        # If KneeLocator is not available or fails, use a simple heuristic
        if optimal_k is None:
            # Calculate the percentage decrease in inertia
            inertia_diffs = np.diff(inertias) / np.array(inertias[:-1])
            # Find where the percentage decrease becomes small
            threshold = 0.1  # 10% threshold
            small_diffs = np.where(np.abs(inertia_diffs) < threshold)[0]
            if len(small_diffs) > 0:
                optimal_k = k_values[small_diffs[0] + 1]  # +1 because diff reduces array size by 1
            else:
                # If no clear elbow, choose a reasonable default
                optimal_k = min(4, len(k_values))
            print(f"Using simple heuristic to find elbow point. Optimal k = {optimal_k}")
        ...
        plt.plot(list(k_values)[:len(inertias)], inertias, 'bo-')
        ...
        plt.close()
        print("OKOKKOOK")
        print(f"K values: {list(k_values)[:len(inertias)]}")
        print(f"Inertias: {inertias}")
        print(f"Optimal number of clusters (Elbow Method): {optimal_k}")

        return optimal_k

    def compute_optimal_k_silhouette(self, adj_matrix, max_k=20):
        """
        Compute the optimal number of clusters using the Silhouette Score.

        The Silhouette Score measures how similar an object is to its own cluster
        compared to other clusters. The higher the score, the better the clustering.
        This method runs k-means clustering for a range of k values and calculates
        the Silhouette Score for each k. The optimal k is the one with the highest
        Silhouette Score.
        ...
        for k in k_values:
            ...
            # Run k-means
            kmeans = KMeans(n_clusters=k, random_state=42, n_init=10)
            cluster_labels = kmeans.fit_predict(X[:, :k-1])  # Use appropriate number of eigenvectors

            # Calculate silhouette score
            # Silhouette score is only defined if there are at least 2 clusters and each cluster has at least 2 samples
            if len(np.unique(cluster_labels)) > 1 and all(np.bincount(cluster_labels) >= 2):
                score = silhouette_score(X[:, :k-1], cluster_labels)
                silhouette_scores.append(score)
            else:
                # If silhouette score can't be calculated, use a very low score
        ...
        optimal_k = list(k_values)[:len(silhouette_scores)][optimal_k_idx]
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(list(k_values)[:len(silhouette_scores)], silhouette_scores, 'bo-')
        plt.xlabel('Number of Clusters (k)')
        ...
        print(f"K values: {list(k_values)[:len(silhouette_scores)]}")
    ...
    def find_good_seed_nodes_fiedler(self, adj_matrix, num_nodes: int = 10) -> List[int]:
        """
        Find good seed nodes using Approximate Fiedler vector localization.

        This method computes the Fiedler vector (second eigenvector of the normalized Laplacian)
        and identifies nodes with extreme values (either very positive or very negative) as
        potential seed nodes for local clustering.
        ...
        """
        print(f"Finding {num_nodes} good seed nodes using Approximate Fiedler vector localization...")
        ...
        # Submatrix for non-isolated nodes
        W_sub = adj_matrix[idx_noniso][:, idx_noniso]
        deg_sub = degrees[idx_noniso]
        ...
        # Eigen-decomposition
        eigvals, eigvecs = eigh(L_sub)
        fiedler_sub = eigvecs[:, 1]  # Second eigenvector (Fiedler vector)

        # Build full-length vector (zeros for isolated nodes)
        fiedler = np.zeros_like(degrees, dtype=float)
        fiedler[idx_noniso] = fiedler_sub

        # Compute the absolute values of the Fiedler vector entries
        fiedler_abs = np.abs(fiedler)

        # Sort nodes by the absolute value of their Fiedler vector entry (descending)
        sorted_nodes = np.argsort(-fiedler_abs)

        # Select the top num_nodes/2 nodes with most positive values
        # and the top num_nodes/2 nodes with most negative values
        positive_nodes = []
        negative_nodes = []

        for node in sorted_nodes:
            if fiedler[node] > 0 and len(positive_nodes) < num_nodes // 2:
                positive_nodes.append(node)
            elif fiedler[node] < 0 and len(negative_nodes) < num_nodes // 2:
                negative_nodes.append(node)

            # Break if we have enough nodes
            if len(positive_nodes) >= num_nodes // 2 and len(negative_nodes) >= num_nodes // 2:
                break

        # If we couldn't find enough nodes in one category, add more from the other
        remaining = num_nodes - len(positive_nodes) - len(negative_nodes)
        if remaining > 0:
            for node in sorted_nodes:
                if node not in positive_nodes and node not in negative_nodes:
                ...
                # Break if we have enough nodes
                if len(positive_nodes) + len(negative_nodes) >= num_nodes:
                    break

        # Combine positive and negative nodes
        seed_nodes = positive_nodes + negative_nodes

        # If we still don't have enough nodes, add more based on absolute value
        if len(seed_nodes) < num_nodes:
            for node in sorted_nodes:
    ...
    def find_good_seed_nodes(self, G: nx.Graph, adj_matrix, motif_name: str, num_nodes: int = 10, method: str = 'combined') -> List[int]:
        ...
        Args:
            ...
            motif_name: Name of the motif
            num_nodes: Number of seed nodes to return (default: 10)
            method: Method to use for ranking nodes ('degree', 'pagerank', 'motif', 'combined', 'fiedler')

        Returns:
            List of node indices that are likely to be good seeds for local clustering
        """
        # If method is 'fiedler', use the Fiedler vector localization method
        if method == 'fiedler':
            return self.find_good_seed_nodes_fiedler(adj_matrix, num_nodes)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clusters', type=int, help='Number of clusters for k-means clustering (if not specified, bisection is used unless --use-eigengap or --use-elbow is specified)')
    ...
    parser.add_argument('--use-elbow', action='store_true', 
                        help='Use Elbow Method to automatically determine the optimal number of clusters')
    parser.add_argument('--use-silhouette', action='store_true', 
                        help='Use Silhouette Score to automatically determine the optimal number of clusters')
    parser.add_argument('--max-k', type=int, default=20, 
                        help='Maximum number of clusters to consider when using eigengap heuristic, Elbow Method, or Silhouette Score (default: 20)')
    ...
    parser.add_argument('--seed-method', type=str, choices=['degree', 'pagerank', 'motif', 'combined', 'fiedler'], default='combined', help='Method to use for ranking nodes when finding good seed nodes (default: combined)')
    ...
    if args.network and args.motif:
        ...
        if args.use_eigengap:
            ...
            optimal_k = motifs.compute_optimal_k_eigengap(adjacency_matrix, args.max_k)
        ...
        # If Elbow Method is requested, compute optimal k
        elif args.use_elbow:
            print(f"Using Elbow Method to determine optimal number of clusters (max_k={args.max_k})...")
            optimal_k = motifs.compute_optimal_k_elbow(adjacency_matrix, args.max_k)

            # Use the optimal k for clustering
            if optimal_k > 1:
                if args.algorithm == 'kmeans':
                    print(f"Performing k-means clustering with {optimal_k} clusters (determined by Elbow Method)...")
                    clusters = motifs.compute_kmeans_clusters(adjacency_matrix, idx_to_node, args.network, args.motif, optimal_k)
                elif args.algorithm == 'minibatch':
                    print(f"Performing mini-batch k-means clustering with {optimal_k} clusters (determined by Elbow Method)...")
                    print(f"Parameters: batch_size={args.batch_size}, max_iter={args.max_iter}, reassignment_ratio={args.reassignment_ratio}")
                    clusters = motifs.compute_minibatch_kmeans_clusters(
                        adjacency_matrix, 
                        idx_to_node, 
                    ...
                    print(f"Performing Gaussian Mixture Model clustering with {optimal_k} clusters (determined by Elbow Method)...")
                    print(f"Parameters: covariance_type={args.covariance_type}, n_init={args.n_init}, max_iter={args.max_iter}")
                    clusters = motifs.compute_gmm_clusters(
                        adjacency_matrix,
                        idx_to_node,
                ...
                # If optimal_k is 1, use traditional bisection
                print("Elbow Method suggests 1 cluster. Performing traditional bisection with Fiedler vector...")
                fiedler = motifs.compute_fiedler(adjacency_matrix, idx_to_node, args.network, args.motif)
                motifs.compute_conductance(adjacency_matrix, fiedler, idx_to_node, args.network, args.motif)

        # If Silhouette Score is requested, compute optimal k
        elif args.use_silhouette:
            print(f"Using Silhouette Score to determine optimal number of clusters (max_k={args.max_k})...")
            optimal_k = motifs.compute_optimal_k_silhouette(adjacency_matrix, args.max_k)

            # Use the optimal k for clustering
            if optimal_k > 1:
                if args.algorithm == 'kmeans':
                    print(f"Performing k-means clustering with {optimal_k} clusters (determined by Silhouette Score)...")
                    clusters = motifs.compute_kmeans_clusters(adjacency_matrix, idx_to_node, args.network, args.motif, optimal_k)
                elif args.algorithm == 'minibatch':
                    print(f"Performing mini-batch k-means clustering with {optimal_k} clusters (determined by Silhouette Score)...")
                    print(f"Parameters: batch_size={args.batch_size}, max_iter={args.max_iter}, reassignment_ratio={args.reassignment_ratio}")
                    clusters = motifs.compute_minibatch_kmeans_clusters(
                        adjacency_matrix, 
                        idx_to_node, 
                    ...
                    print(f"Performing Gaussian Mixture Model clustering with {optimal_k} clusters (determined by Silhouette Score)...")
                    print(f"Parameters: covariance_type={args.covariance_type}, n_init={args.n_init}, max_iter={args.max_iter}")
                    clusters = motifs.compute_gmm_clusters(
                        adjacency_matrix,
                        idx_to_node,
                ...
                # If optimal_k is 1, use traditional bisection
                print("Silhouette Score suggests 1 cluster. Performing traditional bisection with Fiedler vector...")
                fiedler = motifs.compute_fiedler(adjacency_matrix, idx_to_node, args.network, args.motif)
                motifs.compute_conductance(adjacency_matrix, fiedler, idx_to_node, args.network, args.motif)
